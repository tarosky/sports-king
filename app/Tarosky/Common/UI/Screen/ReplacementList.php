...
class ReplacementList extends ScreenBase {
	...
	protected function render() {
		...
		<form id="sk-replacements" action="<?php echo admin_url( 'edit.php' ); ?>" method="get"
				data-endpoint="<?php echo admin_url( 'admin-ajax.php' ); ?>" data-nonce="<?php echo wp_create_nonce( 'sk_edit_repl' ); ?>">
			<input type="hidden" name="post_type" value="team">
			<input type="hidden" name="page" value="sk_replacements">
			<input type="hidden" name="view" value="<?php echo esc_attr( Input::instance()->get( 'view' ) ); ?>">
		<?php
			ob_start();
	...
	public function admin_init() {
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			add_action( 'wp_ajax_sk_replace_edit', array( $this, 'ajax' ) );
		}
	}
